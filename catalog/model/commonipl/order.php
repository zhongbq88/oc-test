...
$sql .="ORDER BY o.date_added DESC LIMIT " . (int)$start . "," . (int)$limit;
	...
	public function updateOrderProduct($order_product_id,$quantity,$options){
		...
		/*$sql = "SELECT price FROM " . DB_PREFIX . "product_sku WHERE product_id='".(int)$query->row['product_id']."' AND product_options like '%" . $this->db->escape($options). "%'";
		print_r($sql);*/
		/*if(!empty($options)){
	...
	public function addOrder($data) {
		...
		$query = $this->db->query("SELECT order_status_id FROM " . DB_PREFIX . "order WHERE forwarded_ip = '" . $this->db->escape($data['forwarded_ip']) . "'");
